...
docker run -it --rm --env-file=k8s/example/server+tls.env enterpriseconnect/plugins:v1 > server+tls.log || cat server+tls.log
printf "\n\n\n*** test client+vln v1 plugin w/ docker\n\n"
docker run -it --rm --env-file=k8s/example/client+vln.env enterpriseconnect/plugins:v1 > client+vln.log || cat client-vln.log
